...
@log_call
def clockin_ss_test():
    log.info("開始執行 Socks5 打卡測試流程...")
    try:
        response = requests.post(
            "https://pro.104.com.tw/psc2/api/f0400/newClockin",
            headers=headers,
            proxies=proxies,  # ✅ 使用 socks5 proxy
            timeout=10
        )
        ...
        if response.status_code == 200 and res_json.get("code") == 200:
            ...
            if att_id:
                msg = f"代理打卡成功！(ID: {att_id})"
                log.info(msg)
                send_telegram_message(f"✅ [VPN] {msg}")
                write_log(msg)
                return True
            else:
        ...
        err = f"VPN 打卡流程發生例外錯誤：{e}"

    log.error(err)
    send_telegram_message(f"❌ VPN 打卡失敗：{err}")
    write_log(err)
    return False

__task_info__ = {
    "name": "clockin_ss_test",
    "desc": "使用 socks5 proxy 嘗試打卡，驗證 VPN 代理是否成功",
    "entry": clockin_ss_test
}
