...
def should_skip_today() -> TaskResult:
    ...
    try:
        if is_today_holiday():
            return TaskResult(code=ResultCode.SKIP_TODAY, message="☀️ 今天是假日或排除日，已略過執行")
        return TaskResult(code=ResultCode.SUCCESS, message="✅ 今天為工作日，可執行任務")
    except Exception as e:
        return TaskResult(code=ResultCode.UNKNOWN_ERROR, message=f"檢查假日邏輯發生錯誤：{e}")
